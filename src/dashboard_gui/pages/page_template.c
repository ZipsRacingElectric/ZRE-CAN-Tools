...
static void styleLoad (pageTemplateStyle_t* style, pageStyle_t* baseStyle, cJSON* config)
{
	*style = (pageTemplateStyle_t)
	{
		.baseStyle = baseStyle
	};
...
page_t* pageTemplateLoad (cJSON* config, canDatabase_t* database, pageStyle_t* style)
{
	...
	// Allocate the page
	pageTemplate_t* page = malloc (sizeof (pageTemplate_t));
	if (page == NULL)
		return NULL;
